...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      stackedWidget(new QStackedWidget(this)),  // ✅ Initialisé dans la liste
      ...
      buttonDelegate(nullptr)
{
    ...
    // Initialisation des vues de graphiques
    ...
    serialManager->connectToPort("COM9");
...
void MainWindow::setupUI() {
  ...
  void MainWindow::onKeypadPressed(char key) {
    if (key == 'A') {
        ...
        QSqlQuery query(db);
        query.prepare("SELECT STOCK FROM RESSOURCESMEDICALES WHERE IDR = :idr");
        query.bindValue(":idr", idStr.toInt());

        if (query.exec()) {
            if (query.next()) {
                QString nom = query.value(0).toString();
                serialManager->getSerial()->write("STOCK:" + nom.toUtf8() + "\n");
                QMessageBox::information(this, "Stock", "STOCK: " + nom);
...
void MainWindow::setupPages() {
    ...
    // === Set Layouts for Pages you want to show text (optional visual debug) ===
    ...
    QLabel *researchesTableLabel = new QLabel("Researches Table Page");
    ...
    setupResearchCards();              // This populates researchesPage
    setupResearchesTablePage();       // This populates researchesTablePage
